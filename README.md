...
## 3. Формирование ETL процесса

В качестве оркестратора данных выбран __Airflow__, а его базой данных __PostgreSQL__
...
* В данной работе были реализованны основные слои __DWH__. 
* Процесс получения, преобразования и загрузки данных (__ETL__ процесс) с помощью оркестратора __Airflow__ был освоен в полной мере.
* Реализованы дашборды в системе визуализации данных __Grafana__.

P.S
Процесс написания дипломной работы был увлекателен. Мой первый серьёзный проект.
Меня переполняют эмоции))). Спасибо всем преподавателям Нетологии за ваш труд.
